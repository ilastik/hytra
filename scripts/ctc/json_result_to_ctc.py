...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert H5 event tracking solution to CTC format",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[
            os.path.abspath("../../hytra/plugins"),
            os.path.abspath("../hytra/plugins"),
        ],
        help="A list of paths to search for plugins for the tracking pipeline.",
    )
    parser.add_argument(
        "--is-ground-truth", dest="is_ground_truth", action="store_true", default=False
    )
    parser.add_argument(
        "--links-to-num-next-frames", dest="linksToNumNextFrames", type=int, default=1
    )
    ...
    trackingGraph = JsonTrackingGraph(
        model_filename=args.model_filename, result_filename=args.result_filename
    )
    ...
    for n in hypothesesGraph.nodeIterator():
        ...
        if "parent" in hypothesesGraph._graph.nodes[n]:
            ...
            trackParents[trackId] = hypothesesGraph._graph.nodes[
                hypothesesGraph._graph.nodes[n]["parent"]
            ]["trackId"]
        if "gap_parent" in hypothesesGraph._graph.nodes[n]:
            assert trackId not in trackParents
            gapTrackParents[trackId] = hypothesesGraph._graph.nodes[
                hypothesesGraph._graph.nodes[n]["gap_parent"]
            ]["trackId"]
    ...
    for trackId, timestepList in tracks.items():
        ...
        if trackId in gapTrackParents.keys():
            if gapTrackParents[trackId] != trackId:
                ...
                logger.info(
                    "Jumping over one time frame in this link: trackid: {}, parent: {}, time: {}".format(
                        trackId, parent, min(timestepList)
                    )
                )
    ...
    timeRange = imageProvider.getTimeRange(
        args.label_image_filename, args.label_image_path
    )
