...
def segmentation_to_hdf5(options):
    ...
    logging.getLogger("segmentation_to_labelimage.py").info(
        "Found image of shape {}".format(data.shape)
    )
    data = hytra.util.axesconversion.adjustOrder(
        data, options.segmentation_axes, "txyzc"
    )
    logging.getLogger("segmentation_to_labelimage.py").info(
        "Changed into shape {}".format(data.shape)
    )

    for timeframe in range(data.shape[0]):
        logging.getLogger("segmentation_to_labelimage.py").info(
            "Working on timestep {}".format(timeframe)
        )
        internalPath = options.hdf5ImagePath % (
            timeframe,
            timeframe + 1,
            data.shape[1],
            data.shape[2],
        ...
        if options.find_connected_components:
            frame = np.expand_dims(
                vigra.analysis.labelMultiArrayWithBackground(
                    data[timeframe, ...].astype(np.uint32)
                ),
                axis=0,
            )
        else:
    ...
    logging.getLogger("segmentation_to_labelimage.py").info(
        "Saved {} timeframes".format(data.shape[0])
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="find_connected_components",
        action="store_true",
    )
    ...
    logging.getLogger("segmentation_to_labelimage.py").debug(
        "Ignoring unknown parameters: {}".format(unknown)
    )
