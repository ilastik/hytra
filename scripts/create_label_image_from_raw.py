...
def filter_labels(a, min_size, max_size=None):
    ...
    """
    if min_size == 0 and (
        max_size is None or max_size > np.prod(a.shape)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Perform the segmentation as in ilastik for a new predicition map,"
    ...
    parser.add_argument(
        ...
        dest="out",
        required=True,
        help="Filename of the resulting HDF5 labelimage",
    )
    parser.add_argument(
        "--overwrite-threshold",
        type=float,
        dest="overwrite_threshold",
        default=None,
    ...
    predictionMaps = hytra.util.axesconversion.adjustOrder(
        predictionMaps, args.prediction_axes, "txyzc"
    )
    ...
    print(
        "Found PredictionMaps of shape {} (txyzc), using channel {}".format(
    ...
    with h5py.File(args.out, "w") as h5file:
        if args.single_volume:
            ...
            for t in range(shape[0]):
                ...
                labelImage = hytra.util.axesconversion.adjustOrder(
                    labelImage, axes, "zyxc"
                )
            ...
            h5file.create_dataset(
                args.labelImagePath, data=fullLabelImage, compression="gzip"
            )
        else:
            # loop over timesteps
            for t in range(shape[0]):
                ...
                labelImage = hytra.util.axesconversion.adjustOrder(
                    labelImage, axes, "txyzc"
                )

                # save
                h5file.create_dataset(
                    args.labelImagePath % (t, t + 1, shape[1], shape[2], shape[3]),
                    data=labelImage,
                    compression="gzip",
                )
                progressBar.show()
