...
def read_positiveLabels(mergers):
    # now: what mergers. Neglabels the not in mergers, only in detections
    print mergers
    quit()
    # gt_mergers =
...
if __name__ == '__main__':
    ...
    jsonTrackingGraph = JsonTrackingGraph(model_filename=args.model_filename, result_filename=args.result_filename)
    model = copy.copy(jsonTrackingGraph.model)
    result = copy.copy(jsonTrackingGraph.result)
    ...
    # now I should have a list of mergers, divisions, links, detections, everything. Here: train on mergers

    read_positiveLabels(mergers)
