...
def remap_label_image(label_images, mapping):
    ...
    """
    remapped_label_image = np.zeros(
        list(label_images.values())[0].shape, dtype=list(label_images.values())[0].dtype
    )
...
def save_frame_to_tif(timestep, label_image, options):
    if options.is_groundtruth:
        filename = (
            options.output_dir
            + "/man_track"
            + format(timestep, "0{}".format(options.filename_zero_padding))
            + ".tif"
        )
    else:
        filename = (
...
def mapGroundTruth(options, hypotheses_graph, trackingGraph, probGenerator):
    ...
    if (
        options.out_obj_count_classifier_file is not None
        and options.out_obj_count_classifier_path is not None
    ):
        ...
        rf.save(
            options.out_obj_count_classifier_file, options.out_obj_count_classifier_path
        )
...
def setupGraph(options):
    ...
    fieldOfView = constructFov(
        ...
        [probGenerator.x_scale, probGenerator.y_scale, probGenerator.z_scale],
    )
...
def run_pipeline(options):
    ...
    if options.load_graph_filename is not None:
        ...
        fieldOfView, hypotheses_graph, ilpOptions, probGenerator, trackingGraph = setupGraph(
            options
        )
    ...
    weights = None
    if (
        options.gt_label_image_file is not None
        and options.gt_label_image_path is not None
        and options.gt_text_file is not None
        and options.gt_jaccard_threshold is not None
    ):
        weights = mapGroundTruth(
            options, hypotheses_graph, trackingGraph, probGenerator
        )
    ...
    for n in hypotheses_graph.nodeIterator():
        ...
        if trackId is not None:
            frameMapping[
                (traxel.idInSegmentation, traxel.segmentationFilename)
            ] = trackId
        ...
        if "parent" in hypotheses_graph._graph.nodes[n]:
            ...
            trackParents[trackId] = hypotheses_graph._graph.nodes[
                hypotheses_graph._graph.nodes[n]["parent"]
            ]["trackId"]
    ...
    pluginManager = TrackingPluginManager(
        verbose=options.verbose, pluginPaths=options.pluginPaths
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--do-convexify", dest="do_convexify", action="store_true", default=False
    )
    parser.add_argument(
        "--plugin-paths",
        dest="pluginPaths",
    ...
    group = parser.add_argument_group(
        "Ground Truth",
        "Specify a ground truth segmentation and tracking which will be used for learning",
    )
    group.add_argument(
        "--gt-label-image-file",
        type=str,
        dest="gt_label_image_file",
        default=None,
        help="Ground Truth Label image filename",
    )
    group.add_argument(
        "--gt-label-image-path",
        dest="gt_label_image_path",
        type=str,
    ...
    group.add_argument(
        "--gt-jaccard-threshold",
        type=float,
        dest="gt_jaccard_threshold",
        default=0.3,
    ...
    # learned classifiers:
    group = parser.add_argument_group(
        "Classifier Output", "Where to save learned classifiers"
    )
    group.add_argument(
        "--out-object-count-classifier-path",
        dest="out_obj_count_classifier_path",
        type=str,
        default="/CountClassification/",
        help="internal hdf5 path to object count probabilities",
    )
    group.add_argument(
        "--out-object-count-classifier-file",
        dest="out_obj_count_classifier_file",
        type=str,
    ...
    # classifiers:
    group = parser.add_argument_group(
        "Classifier Input", "Where to load classifiers from"
    )
    group.add_argument(
        "--object-count-classifier-path",
        dest="obj_count_classifier_path",
        type=str,
    ...
    group.add_argument(
        ...
        dest="obj_count_classifier_file",
        type=str,
        required=True,
        help="Filename of the HDF file containing the object count classifier.",
    )
    group.add_argument(
        "--division-classifier-path",
        dest="div_classifier_path",
        type=str,
    ...
    group.add_argument(
        ...
        dest="transition_classifier_filename",
        type=str,
        default=None,
    )
    group.add_argument(
        "--transition-classifier-path",
        dest="transition_classifier_path",
        type=str,
        default="/",
    )
    ...
    group = parser.add_argument_group(
        ...
        "Provides export for JSON file graph, result, and weights, as well as graph dumping(slow!)",
    )
    group.add_argument(
        "--graph-json-file",
        type=str,
        dest="graph_json_filename",
        default=None,
    ...
    group.add_argument(
        "--result-json-file",
        type=str,
        dest="result_json_filename",
        default=None,
        help="filename where to save the results to in JSON format",
    )
    group.add_argument(
        "--learned-weight-json-file",
        type=str,
        dest="learned_weights_json_filename",
        default=None,
    ...
    group = parser.add_argument_group(
        "Graph and Tracking",
        "Parameters that configure hypotheses graph construction and tracking",
    )
    group.add_argument(
        "--weight-json-file",
        type=str,
        dest="weight_json_filename",
        default=None,
    ...
    group.add_argument(
        "--max-neighbor-distance", dest="max_neighbor_distance", type=float, default=200
    )
    group.add_argument(
        "--max-nearest-neighbors", dest="max_nearest_neighbors", type=int, default=4
    )
    group.add_argument("--with-divisions", dest="with_divisions", action="store_true")
    group.add_argument(
        "--use-flow-solver",
        dest="use_flow_solver",
        action="store_true",
    ...
    group.add_argument(
        "--ctc-filename-zero-pad-length",
        type=int,
        dest="filename_zero_padding",
        default=3,
    )
    ...
    if options.label_image_paths is None or len(options.label_image_paths) == 0:
        options.label_image_paths = [
            "/TrackingFeatureExtraction/LabelImage/0000/[[%d, 0, 0, 0, 0], [%d, %d, %d, %d, 1]]"
        ]
    if (
        len(options.label_image_paths) < len(options.label_image_files)
        and len(options.label_image_paths) == 1
    ):
        options.label_image_paths = options.label_image_paths * len(
            options.label_image_files
        )
