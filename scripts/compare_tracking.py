...
def match(fn_pair):
    ...
    print(
        "-> matched: " + path.basename(fn_pair[0]) + " <-> " + path.basename(fn_pair[1])
    )
    return assoc
...
def getIdsAndValidity(h5file):
    try:
        ...
        print(
            "Warning: could not load ids and validity from hdf5 file. Reconstructing from segmentation..."
        )
...
def construct_associations(base_fns, cont_fns, timesteps, verbose=False):
    ...
    for t in range(timesteps):
        base_fn = base_fns[t]
        cont_fn = cont_fns[t]
        assert int(os.path.splitext(os.path.basename(base_fn))[0]) == int(
            os.path.splitext(os.path.basename(cont_fn))[0]
        )
        ...
        assoc = {
            "lhs": dict(zip(base_ids, cont_ids)),
            "rhs": dict(zip(cont_ids, base_ids)),
        }
        assocs.append(assoc)
    return assocs
...
"""
    ...
    parser.add_option(
        "--quietly", action="store_true", dest="quiet", help="non-verbose"
    )
    parser.add_option(
        "--max-ts",
        dest="max_ts",
        type=int,
        default=-1,
        help="max. timestep (exclusive) [default=%default]",
    )
    ...
    for i, v in enumerate(fn_pairs[1:]):
        ...
        t = quant.compute_taxonomy(
            assocs[i], assocs[i + 1], v[0], v[1], i + first_timestep + 1
        )
        ts.append(t)
