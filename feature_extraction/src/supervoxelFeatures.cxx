...
namespace features
{
    ...
    feature_array extractPosition(three_set &coordinates, feature_array &intensities)
    {
        ...
        //*
        if(ret[3] != 0){
            ret[6] = skew_x/double(size)/std::pow(ret[3],3./2.);
            ret[9] = kurt_x/double(size)/std::pow(ret[3],2) - 3;
        }
        if(ret[4] != 0){
            ret[7] = skew_y/double(size)/std::pow(ret[4],3./2.);
            ret[10] = kurt_y/double(size)/std::pow(ret[4],2) - 3;
        }
        if(ret[5] != 0){
            ret[8] = skew_z/double(size)/std::pow(ret[5],3./2.);
    ...
    feature_array extractWeightedPosition(three_set &coordinates, feature_array &intensities)
    {
        ...
        //*
        if(ret[3] != 0){
            ret[6] = skew_x/double(sum_intensity)/std::pow(ret[3],3./2.);
            ret[9] = kurt_x/double(sum_intensity)/std::pow(ret[3],2) - 3;
        }
        if(ret[4] != 0){
            ret[7] = skew_y/double(sum_intensity)/std::pow(ret[4],3./2.);
            ret[10] = kurt_y/double(sum_intensity)/std::pow(ret[4],2) - 3;
        }
        if(ret[5] != 0){
            ret[8] = skew_z/double(sum_intensity)/std::pow(ret[5],3./2.);
    ...
    feature_array extractIntensity(three_set &coordinates, feature_array &intensities)
    {
        ...
        //*
        if(ret[1] != 0){
            ret[2] = skew/double(size)/std::pow(ret[1],3./2.);
    ...
    feature_array extractPairwise(three_set &coordinates, feature_array &intensities)
    {
        ...
        for(int i = 0; i < volume.size(0)-1; i++){
            for(int j = 0; j < volume.size(1)-1; j++){
                for(int k = 0; k < volume.size(2)-1; k++){
    ...
    feature_array extractSGF(three_set &coordinates, feature_array &intensities)
    {
        ...
        return SGF::SGFeatures(volume, 24);
