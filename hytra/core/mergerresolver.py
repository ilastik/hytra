...
def computeObjectFeatures(labelImages,
                          pluginManager,
                          raw_filename,
                          raw_path,
                          raw_axes,
...
def resolveMergers(
    ...
    if len(mergers) == 0:
        ...
        # graph
        hytra.core.jsongraph.writeToFormattedJSON(out_result, result)
        # result
        hytra.core.jsongraph.writeToFormattedJSON(out_model_filename, model)
        # segmentation
        intTimesteps = [int(t) for t in timesteps]
        intTimesteps.sort()
        imageProvider = pluginManager.getImageProvider()
        ...
        objectFeatures = computeObjectFeatures(labelImages,
                                               pluginManager,
                                               raw_filename,
                                               raw_path,
                                               raw_axes,
        ...
        exportRefinedHypothesesGraph(out_model_filename,
                                     model,
                                     resolvedGraph,
                                     unresolvedGraph,
                                     uuidToTraxelMap,
        ...
        exportRefinedSolution(out_result,
                              result,
                              nodeFlowMap,
                              arcFlowMap,
                              resolvedGraph,
