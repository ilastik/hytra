...
def computeDivisionFeaturesOnCloud(
    frameT,
    featuresAtT,
    featuresAtTPlus1,
    imageProviderPlugin,
...
class IlpProbabilityGenerator(ProbabilityGenerator):
    ...
    def __init__(
        ...
        self._pluginManager = TrackingPluginManager(
            turnOffFeatures=turnOffFeatures, verbose=verbose, pluginPaths=pluginPaths
        )
        ...
        for frame, features in self._featuresPerFrame.items():
            ...
            for objectId in range(1, list(features.values())[0].shape[0]):
                ...
                if self._countClassifier is not None:
                    self._setTraxelFeatureArray(
                        traxel,
                        objectCountProbabilities[objectId, :],
    ...
    def getTransitionFeatureVector(
        ...
        """
        features = np.array(
            self._pluginManager.applyTransitionFeatureVectorConstructionPlugins(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Build a traxelstore from a given ilastik project",
    ...
    probabilityGenerator = IlpProbabilityGenerator(
        ilpOptions=ilpOptions, useMultiprocessing=not args.disableMultiprocessing
    )
