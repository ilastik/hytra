...
"""
    ...
    parser.add_option('--num-threads', type=int, dest='num_threads', default=0,
    ...
    parser.add_option('--gt-path-format', type='string', dest='gt_path_format_string', default="{0:04d}", 
    ...
    parser.add_option('--only-labels', action='store_true', dest='only_labels', help='skip exporting feature and constraint files')
    ...
    consopts.add_option('--max-num-paths', dest='max_num_paths', type='int', default=None, help='Max number of paths DynProg is allowed to find')
    ...
    consopts.add_option('--only-save-first', dest='save_first', action='store_true',
...
def update_merger_features(coordinate_map, h5file, merger_traxel, new_traxel_ids, raw_h5, options, fs, ts, timestep):
    ...
    try:
        for traxel_id in new_traxel_ids:
            ...
            if traxel_id not in label_image_excerpt:
                print("ERROR: could not find requested traxel_id {} in relabeled image {}".format(traxel_id, label_image_excerpt))
        label_image_excerpt -= min_new_traxel_id - 1
    except MemoryError:
        print("WARNING: Something went wrong when updating label image around traxel "
    ...
    try:
        ...
        if ndim == 2:
            ri_ex = ri_ex.squeeze()
        track.extract_region_features_roi(ri_ex,
        ...
        print("WARNING: Something went wrong when extracting region features for traxel "
...
def save_events_parallel(options, all_events, max_traxel_id_at, ilp_file, shape, t0, t1, async=True):
    ...
    for i in range(len(all_events)):
        ...
        processing_pool.apply_async(save_events, (out_dir, copy.copy(events), shape, t0, t1,
...
def train_outlier_svm(options, tracker, ts, fov):
    ...
    with h5py.File(gt_features_file, 'r') as track_features_h5:
        tracks, divisions = trackingfeatures.create_and_link_tracks_and_divisions(track_features_h5,
                                                                                  ts,
                                                                                  getRegionFeatures(ndim))
...
def exportFunkeyFiles(options, ts, tracker, trans_classifier):
    ...
    tracker.writeStructuredLearningFiles(features_filename,
                                         constraints_filename,
                                         labels_filename,
                                         tracking_param)
...
def learn_perturbation_weights(ts, options, shape, trans_classifier, t0, t1):
    ...
    with open(outpath+"/pertubation_labels/pertubation.log", 'w') as f:
        for pertubation_parameter in itertools.product(np.arange(-2, 2, 1), repeat=3):
            ...
            pert_tracker.SetFunkeyOutputFiles('', '', outpath + '/pertubation_labels/perturbed_labeling'+parameter_name+'.txt', False,uncertaintyParam)
            try:
                pert_tracker.track(
                    options.forbidden_cost,
                    options.ep_gap,
    ...
    with h5py.File(ilp_fn, 'r') as h5file:  # open file to access label images
        ...
        for i, event_vector in enumerate(all_events):  # go through all solutions
                ...
                for event in timestep_events:
                    if event.type == track.EventType.Move and event.traxel_idx[1] in mergers:
                        ...
                        try:
                            ...
                            print("Error when extracting coordinates for id={} in timestep={}".format(event.traxel_ids[0], timestep - 1))
            ...
            if with_feature_computation:
                feature_vector, score, feature_names = extract_features_and_compute_score(options.reranker_weight_file, 
...
if __name__ == "__main__":
    ...
    if options.motionModelWeight > 0:
        ...
        params.register_motion_model4_func(swirl_motion_func_creator(options.motionModelWeight), options.motionModelWeight*25.0)
    ...
    if options.with_merger_resolution and options.max_num_objects > 1 and len(options.raw_filename) > 0:
        ...
        try:
            runMergerResolving(options, tracker, ts, fs, hypotheses_graph, ilp_fn, all_events, fov, region_features, trans_classifier, t0, True)
    ...
    if options.save_first:
        ...
        save_events(out_dir, events, shape, t0, t1,
