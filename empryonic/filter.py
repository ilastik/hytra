...
def filterFeaturesByPredicate(h5In, h5Out, predicate):
    ...
    if h5In == h5Out:
        raise ValueError(
            "hdf5 in-place operation not supported (input and output path are identical)"
        )
    ...
    labelGroups = filter(
        lambda item: isinstance(item, h5py.Group), featuresGroup.itervalues()
    )
    ...
    # featureconfig
    outFeaturesGroup.create_dataset(
        featureconfig, data=featuresGroup[featureconfig][()]
    )
    ...
    outFeaturesGroup.create_dataset(
        labelcontent, data=np.array(outLabelcontent, dtype=np.uint16)
    )
