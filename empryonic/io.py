...
def write_visbricks_file(
    h5_filenames, out_dir, visbricks_fn="visbricks_time-dependent-h5.txt"
):
...
def tracklet_from_labelgroup(
    h5_labelgroup, timestep=None, position="mean", add_features_as_meta=True
):
    ...
    if pos_feat not in h5_labelgroup.keys():
        raise Exception(
            position + " feature not present in label group " + str(h5_labelgroup)
        )

    # read out coordinates
    if pos_feat == "com":
        x, y, z = (
            h5_labelgroup[pos_feat][0],
            h5_labelgroup[pos_feat][1],
            h5_labelgroup[pos_feat][2],
        )
    else:
        x, y, z = (
            h5_labelgroup[pos_feat][1],
            h5_labelgroup[pos_feat][2],
            h5_labelgroup[pos_feat][3],
        )

    the_tracklet = _ts.Tracklet(
        x, y, z, timestep, int(_path.basename(h5_labelgroup.name))
    )
...
def ctracklet_from_labelgroup(h5_labelgroup):
    ...
    def add_feature(name, obj):
        if isinstance(obj, h5py.Dataset):
            the_tracklet.add_feature_array(name, len(obj[()]))
...
class LineageH5(h5py.File):
    ...
    def init_tracking(
        self, div=np.empty(0), mov=np.empty(0), dis=np.empty(0), app=np.empty(0)
    ):
    ...
    def update_moves(self, mov_pairs):
        ...
        if len(mov_pairs) > 0:
            self[self.track_gn].create_dataset(
                "Moves", data=np.asarray(mov_pairs, dtype=np.int32)
            )

    def get_moves(self):
        if (
            self.has_tracking()
            and _path.basename(self.mov_ds) in self[self.track_gn].keys()
        ):
    ...
    def get_mergers(self):
        if (
            self.has_tracking()
            and _path.basename(self.merg_ds) in self[self.track_gn].keys()
        ):
    ...
    def get_multiFrameMoves(self):
        if (
            self.has_tracking()
            and _path.basename(self.multi_ds) in self[self.track_gn].keys()
        ):
    ...
    def get_divisions(self):
        if (
            self.has_tracking()
            and _path.basename(self.div_ds) in self[self.track_gn].keys()
        ):
    ...
    def update_divisions(self, div_triples):
        ...
        if len(div_triples) > 0:
            self[self.track_gn].create_dataset(
                "Splits", data=np.asarray(div_triples, dtype=np.int32)
            )
    ...
    def get_disappearances(self):
        if (
            self.has_tracking()
            and _path.basename(self.dis_ds) in self[self.track_gn].keys()
        ):
    ...
    def update_disappearances(self, dis_singlets):
        ...
        if len(dis_singlets) > 0:
            self[self.track_gn].create_dataset(
                "Disappearances", data=np.asarray(dis_singlets, dtype=np.int32)
            )
    ...
    def get_appearances(self):
        if (
            self.has_tracking()
            and _path.basename(self.app_ds) in self[self.track_gn].keys()
        ):
    ...
    def update_appearances(self, app_singlets):
        ...
        if len(app_singlets) > 0:
            self[self.track_gn].create_dataset(
                "Appearances", data=np.asarray(app_singlets, dtype=np.int32)
            )
    ...
    def rm_disappearance(self, id):
        ...
        if not id in diss:
            raise Exception(
                "LineageH5::rm_disappearance(): id %d not an disappearance" % id
            )
    ...
    def Tracklets(self, timestep=None, position="mean", add_features_as_meta=True):
        ...
        features_group = self[self.feat_gn]
        tracklets = _ts.Tracklets(
            [
                tracklet_from_labelgroup(
                    features_group[str(label)],
                    timestep=timestep,
                    add_features_as_meta=add_features_as_meta,
                    position=position,
                )
    ...
    def cTraxels(self, as_python_list=False, prediction_threshold=None):
        if prediction_threshold:
            print(
                "LineageH5::cTraxels: predicition threshold %f" % prediction_threshold
            )
        # probe for objects group (higher io performance than features group)
        if "objects" in self.keys():
            return self._cTraxels_from_objects_group(
                as_python_list, prediction_threshold
            )
    ...
    def _cTraxels_from_objects_group(
        self, as_python_list=False, prediction_threshold=None
    ):
        ...
        class Harvester(object):
            ...
            def __call__(self, name, obj):
                ...
                if name.isdigit():
                    ...
                    name == "featurecontent"
                    or name == "labelcontent"
                    or name == "labelcount"
                ):
                    pass
                else:
                    feature_name = _path.basename(name)
                    self.current_ctracklet.add_feature_array(
                        str(feature_name), len(obj[()])
                    )
                    for i, v in enumerate(obj[()]):
                        self.current_ctracklet.set_feature_value(
                            str(feature_name), i, float(v)
                        )
...
class Test_tracklet_from_labelgroup(ut.TestCase):
    ...
    def test_default(self):
        ...
        self.assertTrue(
            np.all(tr.meta["intmaxpos"] == np.asarray([767.0, 480.0, 591.0, 79.0]))
        )
