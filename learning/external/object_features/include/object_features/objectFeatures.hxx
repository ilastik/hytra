...
namespace features {
    ...
    /** Bounding Box feature
      ...
      * Output stucture:
      * size = 2*N + 1
      * [0 .. N-1] minimum coordinates
      * [N .. 2*N-1] maximum coordinates
        ...
        template <class C, class V> void extract(C coordinates_begin, C coordinates_end, V values_begin) {
            ...
            for(C it = coordinates_begin; it != coordinates_end; it++){
                for(int i = 0; i < N; i++){
                    ...
                    if((*it)[i] > _feature_values[N+i]){
                        _feature_values[N+i] = (*it)[i]; // update maximum
                    }
                }
            }
    ...
    public:
        ...
        /** Constructor to calculate feature immediately
          */
        ObjectIntensity(coordinates_type& coordinates, values_type& values): ObjectFeature<T,N>(4) {
            extract(coordinates, values);
        }
