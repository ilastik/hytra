...
def test_mergerResolvingTestDataset():
    ...
    for f in range(4):
        frame = vigra.impex.readImage(
            "../tests/mergerResolvingTestDataset/ctc_RES/mask00{}.tif".format(f)
        )
        assert np.all(np.unique(frame) == [0, 1, 2])

    with open(
        "../tests/mergerResolvingTestDataset/ctc_RES/res_track.txt", "r"
    ) as resultFile:
...
def test_mergerResolvingTestDatasetNewLabelImage():
    ...
    for f in range(4):
        frame = vigra.impex.readImage(
            "../tests/mergerResolvingTestDatasetNewLabelImage/ctc_RES/mask00{}.tif".format(
                f
            )
        )
        assert np.all(np.unique(frame) == [0, 1, 2])
    ...
    with open(
        "../tests/mergerResolvingTestDatasetNewLabelImage/ctc_RES/res_track.txt", "r"
    ) as resultFile:
        assert resultFile.read() == "1 0 3 0\n2 0 3 0\n"
...
def test_divisionTestDataset():
    ...
    for f in range(4):
        frame = vigra.impex.readImage(
            "../tests/divisionTestDataset/ctc_RES/mask00{}.tif".format(f)
        )
...
def test_skipLinksTestDataset_withoutTracklets():
    ...
    for f in range(4):
        frame = vigra.impex.readImage(
            "../tests/skipLinksTestDataset/ctc_RES/mask00{}.tif".format(f)
        )
...
def test_boxesTestDataset():
    ...
    for f in range(4):
        frame = vigra.impex.readImage(
            "../tests/boxesTestDataset/ctc_RES/mask00{}.tif".format(f)
        )
        assert np.all(np.unique(frame) == expectedIds[f])
...
def test_divisionMergerTestDataset():
    ...
    for f in range(3):
        frame = vigra.impex.readImage(
            "../tests/divisionMergerTestDataset/ctc_RES/mask00{}.tif".format(f)
        )
        assert np.all(np.unique(frame) == expectedIds[f])

    with open(
        "../tests/divisionMergerTestDataset/ctc_RES/res_track.txt", "r"
    ) as resultFile:
