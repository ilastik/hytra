...
def test_lmvnpdf_full():
    ...
    reference = _naive_lmvnpdf_diag(X, mu, cv)
    lpr = log_multivariate_normal_density(X, mu, fullcv, 'full')
    assert_array_almost_equal(lpr, reference)
...
#     g = GMM(n_components, covariance_type, random_state=rng)
...
#     assert_raises(ValueError, GMM, n_components=20,
...
class GMMTester():
    ...
    def test_train(self, params='wmc'):
        g = GMM(n_components=self.n_components,
...
def test_n_parameters():
    ...
    for cv_type in ['full']: #, 'tied', 'diag', 'spherical']:
        g = GMM(n_components=n_components, covariance_type=cv_type,
...
#     X = rng.randn(n_samples, n_dim)
#     g_full = GMM(n_components=n_components, covariance_type='full',
#                          random_state=rng, min_covar=1e-7, n_iter=1)
#     g_full.fit(X)
...
def test_fit_predict():
    ...
    for m_constructor in [GMM]:#, mixture.VBGMM, mixture.DPGMM):
        model = m_constructor(n_components=n_comps, covariance_type='full',
                              min_covar=1e-7, n_iter=5,
                              random_state=np.random.RandomState(0))
        assert_fit_predict_correct(model, X)

    model = GMM(n_components=n_comps, n_iter=0)
...
def check_positive_definite_covars(covariance_type):
    ...
    gmm = GMM(2, params="wc", covariance_type=covariance_type, min_covar=1e-3)
...
def test_verbose_first_level():
    ...
    X = rng.randn(30, 5)
    X[:10] += 2
    g = GMM(n_components=2, n_init=2, verbose=1)
...
def test_verbose_second_level():
    ...
    X = rng.randn(30, 5)
    X[:10] += 2
    g = GMM(n_components=2, n_init=2, verbose=2)
